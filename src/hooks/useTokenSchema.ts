...
export const useCommonToken = () => {
  ...
  const deployParams = async (
    ...
    const { abi, bytecode } = await tokenContracts[
      (tokenType || TokenType.BasicToken) as typeof TokenType.BasicToken
    ]();

    const feeService =
      config.generateTokensPrice[tokenType || TokenType.BasicToken];
...
export const useTaxToken = () => {
  ...
  const deployParams = async (
    ...
    const feeService = config.generateTokensPrice[TokenType.TaxToken];
